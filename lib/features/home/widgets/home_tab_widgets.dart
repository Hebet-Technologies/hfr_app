...
class _JobAdvertisementApplicationsScreenState
  ...
  Future<void> _loadApplications() async {
    ...
    try {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = listLoadErrorMessage(error);
      });
    }
  }
...
class _AnnouncementDetailsScreenState
  ...
  Future<void> _loadComments() async {
    ...
    try {
      ...
      setState(() {
        _isLoadingComments = false;
        _commentsError = listLoadErrorMessage(error);
      });
    }
  }
  ...
  Future<void> _openAttachment(
    ...
    final url = resolveApiFileUrl(attachment.attachmentUrl);
    if (url.isEmpty) {
      return;
    }
...
class _JobAnnouncementApplicationSheetState
  ...
  Future<void> _loadLookups() async {
    ...
    try {
      ...
      var positions = const <RequestLookupOption>[];
      String? positionLookupMessage;
      try {
        positions = await repository.fetchStaffAdvertisementPositions(
          widget.announcement,
        );
      } catch (_) {
        positionLookupMessage =
            'Positions could not be loaded right now. You can continue without selecting a position, or retry.';
      }
      if (!mounted) return;
      ...
      setState(() {
        ...
        _positionLookupMessage = positionLookupMessage;
        if (application != null) {
  ...
  Future<void> _retryPositionLookup() async {
    setState(() => _positionLookupMessage = null);
    try {
      final positions = await ref
          .read(staffRequestsRepositoryProvider)
          .fetchStaffAdvertisementPositions(widget.announcement);
      if (!mounted) return;
      setState(() {
        _positions = positions;
        _selectedPosition = positions.isEmpty ? null : _selectedPosition;
        _positionLookupMessage = positions.isEmpty
            ? 'No positions were returned for this advert. You can continue without selecting a position.'
            : null;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _positionLookupMessage =
            'Still unable to load positions. You can continue without selecting a position.';
      });
    }
  }
  ...
  Future<void> _submit() async {
    ...
    if (_selectedProgram == null) {
      setState(() => _errorMessage = 'Select program.');
      return;
    }
            ...
            : SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (_positionLookupMessage != null) ...[
                      _AnnouncementRetryCard(
                        message: _positionLookupMessage!,
                        onRetry: _retryPositionLookup,
                      ),
                      const SizedBox(height: 12),
                    ],
...
class _AnnouncementRetryCard extends StatelessWidget {
  const _AnnouncementRetryCard({required this.message, required this.onRetry});
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_error != null) ...[
            const SizedBox(height: 16),
            _AnnouncementRetryCard(message: _error!, onRetry: _loadDetail),
          ],
          const SizedBox(height: 20),
          Text(
            'Attachments',
