...
class StaffRequestsRepository {
  ...
  Future<String> submitStaffAdvertisementApplication({
    ...
    final response = await _postForm(
      ...
      data: {
        ...
        if ((position?.id ?? '').trim().isNotEmpty)
  ...
  Future<StaffRequestRecord> submitActivityRequest({
    ...
    final response = (draft.filePath ?? '').trim().isNotEmpty
        ? await _postForm('/staff-activities', data: payload)
        : await _postJson('/staff-activities', data: payload);
    _ensureSuccessfulResponse(
      response,
    ...
    final attachments = _extractListByKey(item, 'attachments')
        .map(
          (attachment) => HomeResourceAttachment(
            ...
            attachmentUrl: resolveApiFileUrl(
              _stringValue(
                attachment['attachment_url'],
                fallback: _stringValue(attachment['file_path']),
              ),
            ),
